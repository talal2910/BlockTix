...
export default function DiscoverPage() {
  const { events, refreshEvents, loading } = useEventStore();
  ...
  // Track the last uid we fetched for — prevents double-fetch caused by
  // Firebase's onIdTokenChanged firing twice (null → uid) on mount.
  const fetchedUidRef = useRef(undefined);
  ...
  // Always fetch fresh recommendations when Discover page opens,
  // but only once per unique uid value.
  useEffect(() => {
  ...
  if (user?.uid && event.category) {
    try {
      await fetch('/api/preferences/click', {
        method: 'POST',
      ...
      // Don't refresh here — we've already navigated away.
      // The click is saved; next time Discover loads, useEffect
      // will call refreshEvents with the updated signals.
    } catch (err) {
      console.error(err);
    }
  }
